...
func (h *Handler) VerifyCode(w http.ResponseWriter, r *http.Request) {
	...
	// The fixed operator code (MULTICA_ADMIN_OTP, admin emails only) bypasses
	// the stored code entirely so it still works when outbound mail is broken
	// and /auth/send-code never produced a row. See auth_password.go.
	if !checkAdminVerificationCode(email, code) {
